	   ...
	   /*private List<TAffichagePpm> ppmTrans = new ArrayList<TAffichagePpm>();
				...
				//Controle Pavé création pour les PRQ
				 if(event.getOldStep().equals("ope111") && event.getNewStep().equals("ope224")) {
		  			 if(strucCond == null || detailPass.getDppStructureBenefi() == null || detailPass.getDppObjet() == null || detailPass.getDppNatInt() == null
		  			   ...
		  			   {
						 FacesContext.getCurrentInstance().addMessage(null,
						 new FacesMessage(FacesMessage.SEVERITY_ERROR, "Veuillez remplir tous les champs obligatoires, avant de cliquer sur suivant!", ""));
				          return "ope111";
						} 
		...
		//Debut controle ANO Technique
		 public void anoTechExiste() {
			if(detailPass.getDppStatutAno().equalsIgnoreCase("O") && detailPass.getDppNbOuv() == 2 ) {
				panelAnoTech = true;     
		 ...
		 public void historiserPs(String statut,String motif) {
			 ...
			 TDetailPlanPassation detailPlan = new TDetailPlanPassation();
			  ...
			  //Historisation des Plans Généraux
			     THistoPlanPassation histoPass = new THistoPlanPassation();
			     histoPass.setHppDate(Calendar.getInstance().getTime());
			     histoPass.setHppMotif(histoPass.getHppMotif());
			     histoPass.setTStatut(statuts);
					...
					FacesContext.getCurrentInstance().addMessage(null,
			 		...
			 		for(VPpmliste lignePpm : listSelectionTransmission) {
			 			...
			 			listePpm =(List<VPpmliste>) iservice.getObjectsByColumn("VPpmliste", new ArrayList<String>(Arrays.asList("DPP_ID")),
							...
							if (!listePpm.isEmpty()) {
								slctdTd = listePpm.get(0); 
								
								if(slctdTd.getCptFinancement().equalsIgnoreCase("0")) {
									FacesContext.getCurrentInstance().addMessage(null,
											new FacesMessage(FacesMessage.SEVERITY_ERROR, "Désolé ! Cette opération du ppm ne possède pas de financement.", ""));
									if(controleController.type == "PPM") {
										chargeData("PN");
								          }else { 
								              if(controleController.type == "PSPM"){
						 			...
						 			listeTsPpm =(List<TDetailPlanPassation>) iservice.getObjectsByColumn("TDetailPlanPassation", new ArrayList<String>(Arrays.asList("DPP_ID")),
									        ...
									        iservice.updateObject(passDetail);
							 			    ...
							 			    //Insertion de chaque ligne dans T_adep_log avec le statut correspondant
											historiser(""+statutTrans,passDetail);
											//Préparation du Tableau de Bord
		     ...
		     //@Transactional
				public void publierSPP()throws IOException{ 
					...
					//Historisation de l'opération
				     historiser(""+statutPub,passDetail);
				     //Préparation du Tableau de Bord
	       ...
	       // @Transactional
			...
			//Affichage des motifs de retour
				public void chargerObservation() {
					ppmstatutList=(List<VPpmStatut>) iservice.getObjectsByColumn("VPpmStatut",
							new WhereClause("HPP_DPP_ID",WhereClause.Comparateur.EQ,""+slctdTd.getDppId()),
				...
				//Methode vider
		    	 public void vider() {
				     ...
				     controleController.lignedefaut= true;
				     ligneftpgpm= false;
				     lignepn= false;
				     ligneps= false;
		    	 }
